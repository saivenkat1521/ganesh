pipeline {
    ...
    stages {
        stage('Delete Old Branches') {
            steps {
                script {
                    def gitRepo = checkout([$class: 'GitSCM', branches: [[name: '*/main']], doGenerateSubmoduleConfigurations: false, extensions: [], submoduleCfg: [], userRemoteConfigs: [[credentialsId: '0a6011eb-9b16-4f3c-9f11-d32b82460219', url: 'https://github.com/your/repo.git']]])
                    ...
                    gitRepo.git('branch -r --format="%(refname:short) %(committerdate:raw)"').eachLine { branch ->
                        ...
                        if (daysDiff > 30 && !branchName.contains('/master')) {
